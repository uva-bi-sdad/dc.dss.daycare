...
data_combined_geos <- data_combined %>% left_join(all_geos, by = "GEOID")
...
# load files
data_combined = read.csv("~/git/dc.dss.daycare/data-raw/data.csv")
data_combined_geos = read_csv("~/git/dc.dss.daycare/data-raw/data_combined_geos.csv")
...
provider_locations = read_csv("~/git/dc.dss.daycare/data-raw/providers.csv")
...
providers_sf <- st_as_sf(providers_locations, coords = c("X", "Y"), crs = 4269)
intersect <- st_intersection(providers_sf, blockgroups)
capacity <- intersect %>% group_by(GEOID) %>% mutate(total_seats = sum(daycares, na.rm = TRUE)) %>% distinct(GEOID, total_seats)
seats$GEOID <- as.numeric(seats$GEOID)
capacity_all <- data_combined_geos %>% left_join(aggregate_hd, by = "GEOID") %>% left_join(seats, by = "GEOID") %>% mutate(total_seats_bg = ifelse(is.na(total_seats), 0, total_seats)) %>% group_by(TRACTCE) %>% mutate(total_seats_tr = sum(total_seats_bg)) %>% group_by(COUNTYFP) %>% mutate(total_seats_ct = sum(total_seats_bg)) %>% group_by(hd_id) %>% mutate(total_seats_hd = sum(total_seats_bg), measure = "daycare_cnt")
...
# 3sfca for tracts
aggregate_tr <- data_combined_geos %>% group_by(TRACTCE) %>% mutate(population_under_15 = sum(population_under_15)) %>% ungroup() %>% mutate(GEOID = paste0("51", COUNTYFP, TRACTCE)) %>% distinct(GEOID, population_under_15) 
...
# 3sfca for counties
aggregate_ct <- data_combined_geos %>% group_by(COUNTYFP) %>% mutate(population_under_15 = sum(population_under_15)) %>% ungroup() %>% mutate(GEOID = paste0("51", COUNTYFP)) %>% distinct(GEOID, population_under_15)
...
aggregate_hd <- data_combined_geos %>% mutate(COUNTYFP = paste0("51", COUNTYFP)) %>% left_join(aggregate_hd, by = "COUNTYFP") %>% group_by(hd_id) %>% mutate(population_under_15 = sum(population_under_15)) %>% select(GEOID, hd_id, population_under_15)
...
# avg travel time (from Hanna) ** need to figure out how we are aggregating these
# find the 5 largest values
list_5_top <- apply(traveltimes, 1, function(x) sort(x, decreasing = F)[1:5])
...
out_df <- left_join(out_df, med_df, by="geoid") %>% mutate(measure = "daycare_median_drive_time_top5")

cnt_bg <- capacity_all %>% ungroup() %>% select(geoid = GEOID, value = total_seats_bg, measure) %>% mutate(region_type = "block group")
cnt_tr <- capacity_all %>% ungroup() %>% select(geoid = GEOID, value = total_seats_tr, measure) %>% mutate(region_type = "tract")
cnt_ct <- capacity_all %>% ungroup() %>% select(geoid = GEOID, value = total_seats_ct, measure) %>% mutate(region_type = "county")
cnt_hd <- capacity_all %>% ungroup() %>% select(geoid = GEOID, value = total_seats_hd, measure) %>% mutate(region_type = "health district")
ca_bg <- data_combined %>% ungroup() %>% select(geoid = GEOID, value = daycare_3sfca) %>% mutate(measure = "daycare_3sfca") %>% mutate(region_type = "block group")
ca_tr <- aggregate_tr %>% select(geoid = GEOID, value) %>% mutate(measure = "daycare_3sfca") %>% mutate(region_type = "tract")
ca_ct <- aggregate_ct %>% select(geoid = GEOID, value) %>% mutate(measure = "daycare_3sfca") %>% mutate(region_type = "county")
ca_hd <- aggregate_hd %>% ungroup() %>% select(geoid = GEOID, value) %>% mutate(measure = "daycare_3sfca") %>% mutate(region_type = "health district")
dt_bg <- out_df %>% select(geoid, value = median_drive_time_top5) %>% mutate(measure = "daycare_median_drive_time_top5", region_type = "block group")

va_hdcttrbg_dss_2021_daycare_access_scores <- cnt_bg %>% rbind(cnt_tr) %>% rbind(cnt_ct) %>% rbind(cnt_hd) %>% rbind(ca_bg) %>% rbind(ca_tr) %>% rbind(ca_ct) %>% rbind(ca_hd) %>% rbind(dt_bg)
